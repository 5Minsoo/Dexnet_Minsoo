...
class GraspPipeline:
    def __init__(self, 
        ...
        self.config_path = os.path.normpath(os.path.join(self.base_dir, "gripper_config.yaml"))
        self.gripper_yaml = os.path.normpath(os.path.join(self.base_dir, "Minsoo_net", "data", "gripper", "gripper.yaml"))
        self.rv_config_path = os.path.normpath(os.path.join(self.base_dir, "Minsoo_net", "config", "random_variables.yaml"))
    ...
    def execute(self):
        ...
        for idx, (pose, prob) in enumerate(zip(stable_poses, probs)):
            ...
            # 2. Quality 평가
            quality_grasps = self.evaluate_grasp_quality(collision_free_grasps, stable_poses)
            ...
            for grasps in aligned_grasps:
                if grasps in quality_grasps:
                    label=1
                else:
                    label=0
            ...
            # 3. 시각화
            visualize_grasps(
                graspable=self.graspable_obj, 
                grasps=collision_free_grasps, # quality_grasps를 보고 싶다면 이 부분을 변경
                pose=pose, 
