...
class Image(object):
    ...
    def _image_data(self):
        ...
        :obj:`numpy.ndarray` of uint8
            ...
            the second is columns, and the third is the R/G/B entry.
        """
        pass
    ...
    def from_array(x, frame="unspecified"):
        """Converts an array of data to an Image based on the values in
        the array and the data format."""
        ...
        if dtype == np.uint8:
            if channels == 1:
                ...
                raise ValueError(
                    "No available image conversion for uint8 array "
                    "with 2 channels"
                )
        elif dtype == np.uint16:
            if channels != 1:
                raise ValueError(
                    "No available image conversion for uint16 array "
                    "with 2 or 3 channels"
                )
            return GrayscaleImage(x, frame)
            ...
            raise ValueError(
                "Conversion for dtype %s not supported!" % (str(dtype))
            )
    ...
    def transform(self, translation, theta, method="opencv"):
        ...
        if method == "opencv":
            ...
            im_data_tf = sni.affine_transform(
                self.data,
                matrix=full_map[:, :2],
                offset=full_map[:, 2],
                order=0,
        ...
        return type(self)(
            im_data_tf.astype(self.data.dtype), frame=self._frame
        )

    def align(self, scale, center, angle, height, width):
        """Create a thumbnail from the original image that
        is scaled by the given factor, centered on the center pixel, oriented
        along the grasp angle, and cropped to the desired height and width.
    ...
    def median_images(images):
        ...
        return type(an_image)(
            median_image_data.astype(an_image.data.dtype), an_image.frame
        )
    ...
    def min_images(images):
        ...
        an_image = images[0]
        return type(an_image)(
            min_image_data.astype(an_image.data.dtype), an_image.frame
        )
    ...
    def __getitem__(self, indices):
        ...
        indices : int or :obj:`tuple` of int
            * int - A linear index.
            * tuple - An ordered index in row, column, and (optionally)
                channel order.
        ...
        if (
            ...
            or (j is not None and isinstance(j, int) and j < 0)
            or (k is not None and isinstance(k, int) and k < 0)
            or (isinstance(i, int) and i >= self.height)
            or (j is not None and isinstance(j, int) and j >= self.width)
            or (k is not None and isinstance(k, int) and k >= self.channels)
        ):
            raise ValueError("Out of bounds indexing")
        if (
            (isinstance(i, slice) and i.start < 0)
            or (j is not None and isinstance(j, slice) and j.start < 0)
            or (k is not None and isinstance(k, slice) and k.start < 0)
            or (isinstance(i, slice) and i.stop > self.height)
            or (j is not None and isinstance(j, slice) and j.stop > self.width)
            or (
                k is not None
                and isinstance(k, slice)
                and k.stop > self.channels
            )
        ):
    ...
    def crop(self, height, width, center_i=None, center_j=None):
        ...
        center_i : int
            The center height point at which to crop. If not specified,
    ...
    def focus(self, height, width, center_i=None, center_j=None):
        ...
        center_j : int
            The center width point of the crop box. If not specified,
            the center of the image is used.
        ...
        ] = self._data[start_row : end_row + 1, start_col : end_col + 1]
    ...
    def center_nonzero(self):
        ...
        nonzero_px_tf[:, 0] = np.max(
            np.c_[np.zeros(nonzero_px_tf[:, 0].shape), nonzero_px_tf[:, 0]],
            axis=1,
        )
        nonzero_px_tf[:, 0] = np.min(
            np.c_[
                (self.height - 1) * np.ones(nonzero_px_tf[:, 0].shape),
                nonzero_px_tf[:, 0],
            ],
            axis=1,
        )
        nonzero_px_tf[:, 1] = np.max(
            np.c_[np.zeros(nonzero_px_tf[:, 1].shape), nonzero_px_tf[:, 1]],
            axis=1,
        )
        nonzero_px_tf[:, 1] = np.min(
            np.c_[
                (self.width - 1) * np.ones(nonzero_px_tf[:, 1].shape),
                nonzero_px_tf[:, 1],
            ],
            axis=1,
        )
        ...
        shifted_data[nonzero_px_tf[:, 0], nonzero_px_tf[:, 1], :] = self.data[
            nonzero_px[:, 0], nonzero_px[:, 1]
        ].reshape(-1, self.channels)

        return (
            type(self)(
                shifted_data.astype(self.data.dtype), frame=self._frame
            ),
            diff_px,
        )
    ...
    def savefig(self, output_path, title, dpi=400, format="png", cmap=None):
        ...
        plt.savefig(
            os.path.join(
                output_path, "{0}.{1}".format(title_underscore, format)
            ),
            dpi=dpi,
            format=format,
        )
...
class ColorImage(Image):
    ...
    def resize(self, size, interp="bilinear"):
        ...
        :obj:`ColorImage`
            The resized image.
        """
        resized_data = imresize(self.data, size, interp=interp).astype(
            np.uint8
        )
    ...
    def mask_binary(self, binary_im):
        ...
        binary_im : :obj:`BinaryImage`
            A BinaryImage of the same size as this image, with pixel values
            of either zero or one. Wherever this image has zero pixels,
            we'll zero out the pixels of the new image.
    ...
    def foreground_mask(
        ...
        a uniformly colored background. The background is assumed to be the
        mode value of the histogram for each of the color channels.
        ...
        scale : int
            Size of background histogram bins -- there will be
            BINARY_IM_MAX_VAL/size bins in the color histogram
            for each channel.
        ...
        if bgmodel is None:
            bgmodel = self.background_model(
                ignore_black=ignore_black, use_hsv=use_hsv, scale=scale
            )
    ...
    def background_model(self, ignore_black=True, use_hsv=False, scale=8):
        ...
        ----------
        ignore_black : bool
            If True, the zero pixels     def gradient_threshold_sobel(self, threshold):
        ...
        return edge_pixelswill be ignored
            when computing the background model.
    ...
    def segment_kmeans(self, rgb_weight, num_clusters, hue_weight=0.0):
        """
        Segment a color image using KMeans based on spatial and color
        distances. Black pixels will automatically be assigned to their
        own 'background' cluster.
        ...
        color_vals = (
            rgb_weight * self._data[nonzero_px[:, 0], nonzero_px[:, 1], :]
        )
        if hue_weight > 0.0:
            hsv_data = cv2.cvtColor(self.data, cv2.COLOR_BGR2HSV)
            color_vals = np.c_[
                color_vals,
                hue_weight * hsv_data[nonzero_px[:, 0], nonzero_px[:, 1], :1],
            ]
    ...
    def inpaint(self, win_size=3, rescale_factor=1.0):
        ...
        rescale_factor : float
            amount to rescale the image for inpainting, smaller numbers
            increase speed
        ...
        inpainted_data = cv2.inpaint(
            resized_data, mask.astype(np.uint8), win_size, cv2.INPAINT_TELEA
        )
...
class DepthImage(Image):
    ...
    def _check_valid_data(self, data):
        ...
        """
        if (
            data.dtype.type is not np.float32
            and data.dtype.type is not np.float64
        ):
            raise ValueError(
    ...
    def _image_data(
        ...
        """Returns the data in image format, with scaling and conversion to
        uint8 types.
        ...
        if normalize:
            ...
            depth_data = (
                (self._data - min_depth)
                * (float(max_val) / (max_depth - min_depth))
            ).squeeze()
    ...
    def threshold_gradients_pctile(self, thresh_pctile, min_mag=0.0):
        ...
        ind = np.where(
            (gradient_mags > grad_thresh) & (gradient_mags > min_mag)
        )
        data[ind[0], ind[1]] = 0.0
        return DepthImage(data, self._frame)
    ...
    def inpaint(self, rescale_factor=1.0):
        ...
        zeros = cur_data == 0
        while np.any(zeros):
            neighbors = ssg.convolve2d(
                (cur_data != 0), inpaint_kernel, mode="same", boundary="symm"
            )
            avg_depth = ssg.convolve2d(
                cur_data, inpaint_kernel, mode="same", boundary="symm"
            )
            avg_depth[neighbors > 0] = (
                avg_depth[neighbors > 0] / neighbors[neighbors > 0]
            )
    ...
    def invalid_pixel_mask(self):
        ...
        -------
        :obj:`BinaryImage`
            Binary image where a pixel value greater than zero indicates an
            invalid pixel.
        """
        # init mask buffer
    ...
    def mask_binary(self, binary_im):
        ...
        ----------
        binary_im : :obj:`BinaryImage`
            A BinaryImage of the same size as this image, with pixel values of
            either zero or one. Wherever this image has zero pixels, we'll
            zero out the pixels of the new image.

        Returns
        -------
        :obj:`DepthImage`
            A new DepthImage of the same type, masked by the
            given binary image.
        """
        data = np.copy(self._data)
        ind = np.where(binary_im.data == 0)
    ...
    def pixels_farther_than(self, depth_im, filter_equal_depth=False):
        ...
        if filter_equal_depth:
            farther_px = np.where(
                (self.data > depth_im.data) & (np.isfinite(depth_im.data))
            )
        else:
            farther_px = np.where(
                (self.data >= depth_im.data) & (np.isfinite(depth_im.data))
    ...
    def combine_with(self, depth_im):
        """
        Replaces all zeros in the source depth image with the value of a
        ...
        ] = depth_im.data[(new_data > depth_im.data) & (depth_im.data > 0)]
        return DepthImage(new_data, frame=self.frame)
    ...
    def point_normal_cloud(self, camera_intr):
        ...
        return PointNormalCloud(
            point_cloud.data, normal_cloud.data, frame=self._frame
        )
...
class IrImage(Image):
    ...
    def _image_data(self):
        ...
        :obj:`numpy.ndarray` of uint8
            A 3D matrix representing the image. The first dimension is rows,
            the second is columns, and the third is simply the IR entry
...
class GrayscaleImage(Image):
    """A grayscale image in which individual pixels have a single
    uint8 channel.
    """
...
class BinaryImage(Image):
    ...
    def __init__(
        ...
        data = BINARY_IM_MAX_VAL * (data > threshold).astype(
            data.dtype
        )  # binarize
        Image.__init__(self, data, frame)
    ...
    def _check_valid_data(self, data):
        ...
        if data.dtype.type is not np.uint8:
            raise ValueError(
                "Illegal data type. \
                    Binary images only support 8-bit uint arrays"
            )

        if len(data.shape) == 3 and data.shape[2] != 1:
            raise ValueError(
                "Illegal data type. \
                    Binary images only support single channel "
            )
    ...
    def _image_data(self):
        ...
        :obj:`numpy.ndarray` of uint8
            A 3D matrix representing the image. The first dimension is rows,
            the second is columns, and the third is simply the binary
            0/BINARY_IM_MAX_VAL value.
        """
        return self._data.squeeze()
    ...
    def resize(self, size, interp="bilinear"):
        ...
        resized_data = imresize(
            self.data.astype(np.float32), size, interp=interp
        )
    ...
    def prune_contours(
        self, area_thresh=1000.0, dist_thresh=20, preserve_topology=True
    ):
        ...
        for i in range(num_contours):
            ...
            if area > min_area and area < max_area:
                ...
                kept_contours.append(
                    Contour(
                        boundary_px_ij_swapped, area=area, frame=self._frame
                    )
                )
    ...
    def contour_mask(self, contour):
        ...
        cv2.fillPoly(
            new_data,
            pts=[boundary_px_ij_swapped.astype(np.int32)],
            color=(BINARY_IM_MAX_VAL, BINARY_IM_MAX_VAL, BINARY_IM_MAX_VAL),
        )
        orig_zeros = np.where(self.data == 0)
    ...
    def boundary_map(self):
        ...
        for contour in contours:
            new_data[
                contour.boundary_pixels[:, 0].astype(np.uint8),
                contour.boundary_pixels[:, 1].astype(np.uint8),
    ...
    def closest_pixel_to_set(self, start, pixel_set, direction, w=13, t=0.5):
        ...
        w : int
            ...
            As soon as the current pixel has some non-zero pixel with a
            diameter w of it, this function returns the current
            pixel location.
        ...
        if (
            np.all(cur_px_y >= 0)
            and np.all(cur_px_y < self.height)
            and np.all(cur_px_x >= 0)
            and np.all(cur_px_x < self.width)
        ):
    ...
    def add_frame(
        self, left_boundary, right_boundary, upper_boundary, lower_boundary
    ):
        ...
        right_boundary : int
            the rightmost boundary of the frame (must be greater than
            left_boundary)
        upper_boundary : int
            the upper boundary of the frame
        lower_boundary : int
            the lower boundary of the frame (must be greater than
            upper_boundary)
    ...
    def to_distance_im(self):
        ...
        -------
        :obj:`numpy.ndarray`
            HxW float array containing the distance transform of
            the binary image
        """
...
class RgbdImage(Image):
    ...
    def from_color_and_depth(color_im, depth_im):
        ...
        # check shape
        if (
            color_im.height != depth_im.height
    ...
    def color(self):
        ...
        return ColorImage(
            self.raw_data[:, :, :3].astype(np.uint8), frame=self.frame
        )
    ...
    def crop(self, height, width, center_i=None, center_j=None):
        ...
        color_im_cropped = self.color.crop(
            height, width, center_i=center_i, center_j=center_j
        )
        depth_im_cropped = self.depth.crop(
            height, width, center_i=center_i, center_j=center_j
        )
    ...
    def combine_with(self, rgbd_im):
        ...
        depth_replace_px = np.where(
            (other_depth_data != 0) & (other_depth_data < depth_data)
        )
        depth_replace_px = np.c_[depth_replace_px[0], depth_replace_px[1]]

        # replace zero pixels
        new_data[depth_zero_px[:, 0], depth_zero_px[:, 1], :] = rgbd_im.data[
            depth_zero_px[:, 0], depth_zero_px[:, 1], :
        ]

        # take closest pixel
        new_data[
            depth_replace_px[:, 0], depth_replace_px[:, 1], :
...
class GdImage(Image):
    ...
    def from_grayscale_and_depth(gray_im, depth_im):
        ...
        # check shape
        if (
            gray_im.height != depth_im.height
            or gray_im.width != depth_im.width
        ):
            raise ValueError(
...
class SegmentationImage(Image):
    ...
    def _check_valid_data(self, data):
        ...
        if (
            data.dtype.type is not np.uint8
            and data.dtype.type is not np.uint16
        ):
            raise ValueError(
                "Illegal data type. Segmentation images only support \
                    8-bit or 16-bit uint arrays"
            )

        if len(data.shape) == 3 and data.shape[2] != 1:
            raise ValueError(
                "Illegal data type. \
                    Segmentation images only support single channel."
            )
    ...
    def border_pixels(
        self, grad_sigma=0.5, grad_lower_thresh=0.1, grad_upper_thresh=1.0
    ):
        """
        Returns the pixels on the boundary between all segments,
        excluding the zero segment.
        ...
        grad_lower_thresh : float
            lower threshold on gradient threshold used to determine
            the boundary pixels
        grad_upper_thresh : float
            upper threshold on gradient threshold used to determine
            the boundary pixels
        ...
        for i in range(1, self.num_segments):
            ...
            nonborder_px = np.where(
                (grad_mag < grad_lower_thresh) | (grad_mag > grad_upper_thresh)
            )
    ...
    def segment_mask(self, segnum):
        """Returns a binary image of just the segment corresponding
        to the given number.
    ...
    def mask_binary(self, binary_im):
        ...
        binary_im : :obj:`BinaryImage`
            A BinaryImage of the same size as this image, with pixel values
            of either zero or one. Wherever this image has zero pixels, we'll
            zero out the pixels of the new image.
    ...
    def to_color(self):
        if self.num_segments > 10:
            ...
            color = (np.iinfo(np.uint8).max * plt.cm.tab10(self.data)).astype(
                np.uint8
            )
...
class PointCloudImage(Image):
    """A point cloud image in which individual pixels
    have three float channels."""
    ...
    def _check_valid_data(self, data):
        ...
        if (
            ...
            raise ValueError(
                "Illegal data type. PointCloud images only \
                    support 32-bit or 64-bit float arrays"
            )
    ...
    def resize(self, size, interp="nearest"):
        ...
        :obj:`PointCloudImage`
            The resized image.
        """
        resized_data_0 = imresize(
            self._data[:, :, 0], size, interp=interp
        ).astype(np.float32)
        resized_data_1 = imresize(
            self._data[:, :, 1], size, interp=interp
        ).astype(np.float32)
        resized_data_2 = imresize(
            self._data[:, :, 2], size, interp=interp
        ).astype(np.float32)
        resized_data = np.zeros(
            [resized_data_0.shape[0], resized_data_0.shape[1], self.channels]
        )
    ...
    def to_mesh(self, dist_thresh=0.01):
        ...
        vertices = []
        triangles = []
        vertex_indices = -1 * np.ones([self.height, self.width]).astype(
            np.int32
        )
    ...
    def to_point_cloud(self):
        ...
        """
        return PointCloud(
            data=self._data.reshape(self.height * self.width, 3).T,
            frame=self._frame,
        )
    ...
    def normal_cloud_im(self, ksize=3):
        ...
        pc_grads[pc_grad_norms == 0.0] = np.array(
            [0, 0, -1.0]
...
class NormalCloudImage(Image):
    """A normal cloud image in which individual pixels
    have three float channels."""
    ...
    def _check_valid_data(self, data):
        ...
        if (
            ...
            raise ValueError(
                "Illegal data type. NormalCloud images only \
                    support 32-bit or 64-bit float arrays"
            )
        ...
        if len(data.shape) != 3 or data.shape[2] != 3:
            raise ValueError(
                "Illegal data type. \
                    NormalCloud images must have three channels"
            )

        if np.any(
            (np.abs(np.linalg.norm(data, axis=2) - 1.0) > 1e-4)
